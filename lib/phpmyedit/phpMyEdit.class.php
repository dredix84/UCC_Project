...
class phpMyEdit
{
	...
	var $urlintsymb = "&";	//What character should be used as the initial character when creating a URL ? or &
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function list_table() /* {{{ */
	{
		...
		echo '<table class="',$this->getCSSclass('main'),'" summary="',$this->tb,'"><thead>',"\n";
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			...
			if (! $this->fdd[$fd]['sort'] || $this->password($fd)) {
				...
				echo htmlspecialchars($this->page_name.$this->urlintsymb.$this->cgi['prefix']['sys'].'fm'.'=0'
		...
		if ($qparts['orderby'] && $this->display['sort']) {
			...
			echo htmlspecialchars($this->page_name
					.$this->urlintsymb.$this->cgi['prefix']['sys'].'fl'.'='.$this->fl
		...
		if ($this->nav_text_links() || $this->nav_graphic_links()) {
			...
			$qpviewStr   = htmlspecialchars($this->page_name.$this->urlintsymb.join('&',$qpview).$this->qfn);
			$qpcopyStr   = htmlspecialchars($this->page_name.$this->urlintsymb.join('&',$qpcopy).$this->qfn);
			$qpchangeStr = htmlspecialchars($this->page_name.$this->urlintsymb.join('&',$qpchange).$this->qfn);
			$qpdeleteStr = htmlspecialchars($this->page_name.$this->urlintsymb.join('&',$qpdelete).$this->qfn);
		}
	...
	function phpMyEdit($opts) /* {{{ */
	{
		...
		$this->sql_delimiter();
		if(isset($opts['preurl'])){$this->urlintsymb = $opts['preurl'];}else{$this->urlintsymb = "&";}
		if (isset($opts['dbh'])) {
